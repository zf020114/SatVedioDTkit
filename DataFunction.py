...
def get_file_paths_recursive_ICPR(father_folder=None, file_ext=None):
    """ Get the absolute path of all files in given folder recursively
    :param folder:
    :param file_ext:
    :return:
    """
    file_list = []
    if father_folder is None:
        return file_list

    for root, dirs, files in os.walk(father_folder, topdown=False):
        for name in files:
...
#    head=headstr.format(name,foldername,size[1],size[0])
    ...
    for i,box in enumerate (gtbox_label):
        ...
        obj=objstr_vocxml.format(CLASSES[int(box[4])],0,box[0],box[1],box[2],box[3],0.3+0.7*np.random.random())
#        obj=objstr.format(labelMat[i],difficultMat[i],center[0],center[1],NewWidth,NewHeight,Angle)
        f.write(obj)
    f.write(tailstr)
    f.close() 

def write_result_VOC_xml(output_floder,img_name,size,gsd,imagesource,gtbox_label,CLASSES):
    #将检测结果表示为VOC格式的xml文件 结果文件需要将置信度 放入 extra字段
...
#    head=headstr.format(name,foldername,size[1],size[0])
    ...
    for i,box in enumerate (gtbox_label):
        # obj=objstr_vocxml.format(CLASSES[int(box[4])],box[5],box[0],box[1],box[2],box[3])
        obj=objstr_vocxml.format(CLASSES[int(box[4])],0,box[0],box[1],box[2],box[3],box[5])
#        obj=objstr.format(labelMat[i],difficultMat[i],center[0],center[1],NewWidth,NewHeight,Angle)
        f.write(obj)
    f.write(tailstr)
    f.close() 
